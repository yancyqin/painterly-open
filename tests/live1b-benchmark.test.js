...
test("1B additive benchmark has one visible canvas and tabbed stages", () => {
  ...
  assert.match(html, /id="bench-mark-limit"[^>]*max="6140"/);
  ...
  assert.match(source, /curveRole: "cover"/);
  assert.match(source, /curveRole: "moving"/);
...
test("1B benchmark uses the owner-final project and supports actor dragging", () => {
  ...
  assert.match(source, /project\.stats\.marks !== 6140/);
